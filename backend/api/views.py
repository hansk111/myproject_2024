...
def index(request):
    return render(None, os.path.join(settings.STATIC_DIR, 'common/index.html'))
...
def upload_image(request):
    try:
        print("upload_image")
        response = Image.upload(DjangoAdapter(request), '/media/postcontent/')
        response['link'] = 'http://localhost:8000/' + response['link']
        print(response)
    except Exception:
        response = {'error': str(sys.exc_info()[1])}
    return HttpResponse(json.dumps(response), content_type="application/json")
